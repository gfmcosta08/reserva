...
// ===== FECHAR CAUTELA (forçado, p/ cautelas vazias ou cancelamento manual) =====
export async function closeCautela(cautelaId: string) {
  const idParsed = uuidSchema.safeParse(cautelaId)
  if (!idParsed.success) {
    return { error: "Identificador de cautela inválido" }
  }
  ...
  if (updateError) {
    return { error: "Erro ao fechar cautela: " + updateError.message }
  }
