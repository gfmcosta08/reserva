...
**Versão:** 1.3  
**Data:** 2026-05-18  
**Status:** Documentação do produto (com requisitos de cadastro de pessoa atualizados)
...
Além disso, para **assinatura na cautela**: PIN de 4 dígitos (`pin_hash`) — ver §5.3. Biometria facial permanece **opcional**.
...
### 7.3 Saldo por quantidade (parcial na mesma linha)

Quando `quantity_delivered > 1` (ex.: 3 carregadores, 50 munições), a devolução pode ser **parcial na mesma linha** de `cautela_items`:

| Situação | `cautela_items.status` | `quantity_returned` | Status da **cautela** |
|----------|------------------------|---------------------|------------------------|
| Devolveu 2 de 3 carregadores (ainda deve 1) | `pending` | `2` | `partial` — **não** `divergent` |
| Devolveu os 3 carregadores | `returned` | `3` | `partial` ou `closed` conforme demais itens |
| Item danificado ou extraviado | `damaged` ou `missing` | `0` | `divergent` |
| Pistola devolvida (qty 1) | `returned` | `1` | avança conforme outros itens |

**Regras:**

- **`divergent`** apenas para `damaged` / `missing` — saldo de quantidade **não** é divergência.
- Linha com saldo permanece **`pending`** até `quantity_returned === quantity_delivered`.
- Cautela **`partial`** permanece aberta (`closed_at` nulo) enquanto houver linha com saldo ou item `pending`.
- Material com saldo em custódia permanece **`cautelado`**; só `available` quando a linha estiver quitada.
- UI: por linha, operador escolhe **Devolver total** ou **Devolver parcial** (quantidade acumulada); inventário agrupado por Armas / Carregadores / Munição / Demais.
...
*Documento atualizado em 2026-05-18 — v1.3: campos obrigatórios de cadastro de pessoa (§5.1).*
