...
**Versão:** 1.4  
**Data:** 2026-06-06  
**Status:** Documentação do produto (ambiente QA, importação legado 1º BPM, UX materiais e regularização de cadastro)
...
### 1.3 Ambientes (governança teste → produção)

| Ambiente | URL | Banco Supabase (`supabaseRef`) | Uso |
|----------|-----|--------------------------------|-----|
| **Produção** | https://reserva1bpm.vercel.app | `mxlgkpfiugbodocyleij` | Operação real |
| **Teste / QA** | https://reserva-teste.vercel.app | `ajyvznrmbuistlcfckuh` (`teste_db`) | **Validação obrigatória** antes de promover |

Confirmação rápida: `GET /api/version` retorna `supabaseEnv`, `supabaseRef` e `vercelEnv`.

**Regra de release:** nenhuma mudança vai para produção sem validação e aprovação explícita no ambiente de teste. Scripts locais: `scripts/switch-env-test.ps1`, `scripts/switch-env-production.ps1`. Documentação operacional: `docs/VERCEL-AMBIENTES.md`, `docs/CHANGE-PROTOCOL.md`.
...
| rg | text | **obrigatório** — número do RG (único) |
| registration_number | text | **obrigatório** — matrícula funcional (único) |
| cpf | text | **obrigatório** — 11 dígitos, único (somente números) |
| phone | text | **obrigatório** — WhatsApp/telefone (notificações de cautela) |
| function | text | cargo/função (opcional) |
| status | enum | `active` \| `inactive` |
| pin_hash | text | bcrypt (custo 10) do PIN de 4 dígitos — **obrigatório** para assinatura na cautela |
| rg_front_url | text | **obrigatório** — foto frente do RG (URL no Storage) |
| rg_back_url | text | **obrigatório** — foto verso do RG (URL no Storage) |
| face_descriptor | JSONB | vetor 128D do face-api.js (opcional) |
...
### 4.5 UX da listagem e detalhe de cautela ativa (v1.4)

```
[Tela /materials]
  ├── Formulário modal (MaterialForm):
  │     ├── max-h-[92dvh] com corpo rolável e rodapé fixo
  │     ├── Fecha com Escape ou clique no backdrop
  │     └── Botão "Voltar" visível em mobile
  │
  ├── Listagem responsiva (MaterialsClient):
  │     ├── Cards em mobile/tablet; tabela em desktop
  │     └── Badge status "Em Uso" (cautelado) é clicável
  │
  └── Clique em "Em Uso" → MaterialCautelaDetailModal
        ├── Pessoa com o material
        ├── Operador que criou a cautela
        ├── Data/hora de saída
        └── Link para /cautelas?id=<cautelaId>
```

**Dados:** `lib/material-active-detail.ts` mapeia material → cautela ativa consultando `cautela_items` com `status`, `quantity_delivered` e `quantity_returned` (via `itemNeedsReturn()` em `lib/cautela-return-status.ts`). Enriquecimento em `lib/materials-page-data.ts`.

> **Correção v1.4:** a coluna `cautela_items.returned` **não existe** no schema; o detalhe "Em Uso" usa a lógica de devolução parcial já adotada no fluxo de retorno.

---

## 5. Cadastro de Pessoas

### 5.1 Campos obrigatórios (política v1.3)

Nenhuma pessoa pode ser considerada **cadastrada** nem participar de cautela até possuir **todos** os itens abaixo:

| # | Campo | Onde no sistema |
|---|--------|-----------------|
| 1 | Nome completo | `persons.full_name` |
| 2 | Matrícula funcional | `persons.registration_number` |
| 3 | RG | `persons.rg` |
| 4 | CPF | `persons.cpf` |
| 5 | E-mail | `persons.email` |
| 6 | WhatsApp (telefone) | `persons.phone` |
| 7 | Foto frente do RG | `persons.rg_front_url` |
| 8 | Foto verso do RG | `persons.rg_back_url` |

Além disso, para **assinatura na cautela**: PIN de 4 dígitos (`pin_hash`) — ver §5.2. Biometria facial permanece **opcional**.

Importações em lote (ex.: legado 1º BPM) podem criar registro **incompleto** apenas em ambiente de teste, com obrigação de completar no balcão antes de operação em produção.

**Pendências operacionais exibidas na UI (v1.4):** além dos campos §5.1, o sistema sinaliza pendência quando faltam **fotos do RG** (`rg_front_url` / `rg_back_url`) ou **biometria facial** (`face_descriptor` vazio). Helper: `lib/person-registration-status.ts` (`personHasRegistrationPending`, `personMissingRgPhotos`, `personMissingFace`).

### 5.2 Fluxo de Registro (novo cadastro)
...
[Tela /persons] → Botão: "+ Nova Pessoa"
  └── Wizard de Cadastro (PersonRegistrationWizard)
        ...
        │     ├── e-mail* (único)
        │     ├── WhatsApp* (persons.phone)
        │     ├── função/cargo (opcional)
        │     └── PIN de 4 dígitos* (digitado 2x — assinatura na cautela)
        │           └── Hash: bcrypt(pin, custo=10) → salvo em pin_hash
        │
        ├── Passo 2: Fotos do RG (obrigatório)
        │     ├── Upload frente do RG*
  ...
  └── Botão: Salvar (bloqueado se faltar qualquer campo obrigatório §5.1)
        └── Action: createPerson(data)
              ├── Valida unicidade: email, rg, registration_number, cpf
              ├── Exige rg_front_url e rg_back_url preenchidos
              ├── Grava todos os campos em `persons`
              ├── Cria entrada em `audit_logs`
              └── Revalida cache: /persons
```

### 5.3 Segurança do PIN
...
- Na **regularização**, só preenche campos de foto ainda vazios (`regularizePerson` não sobrescreve URL existente)

### 5.5 Regularização de cadastro pendente (v1.4)

Dois caminhos equivalentes abrem o wizard em modo **"Regularizar Cadastro"** (`PersonRegistrationWizard` com prop `person`):

#### Caminho A — Gestão de Pessoas
```
[Tela /persons]
  ├── Coluna Docs: badge "Pendente" (amarelo) se personHasRegistrationPending
  │     └── Clique no badge → abre wizard de regularização
  ├── Ícone editar (lápis) → mesmo wizard
  └── URL /persons?edit=<personId> → abre wizard automaticamente (deep link)
```

#### Caminho B — Nova Cautela
```
[CautelaWizard — Passo 1: Selecionar Pessoa]
  ├── Ao selecionar pessoa com pendências → caixa amarela:
  │     ├── "Fotos do RG incompletas" (se aplicável)
  │     └── "Biometria facial não cadastrada" (se aplicável)
  └── Link "Regularizar cadastro" → modal com PersonRegistrationWizard
        └── Ao salvar: atualiza selectedPerson in-place (sem sair do fluxo)
```

**Wizard em modo regularização:**
- Pré-preenche dados, fotos já cadastradas e biometria existente
- RG e matrícula **somente leitura**
- PIN opcional (importados legado usam PIN temporário `0000`)
- Passo inicial inteligente: fotos faltantes → passo 1; só biometria → passo 4
- Submit: `regularizePerson(id, data)` em `app/actions/persons.ts`
- Leitura prévia: `getPersonById(id)`
...
[Busca de pessoa]
  ...
  └── Selecionar pessoa → avança para Passo 2
        ├── Verifica cautelas diárias pendentes (getPendingCautelasForPerson)
        ├── Pendências cadastrais (fotos RG / biometria) → aviso + "Regularizar cadastro"
        └── Se sem PIN e sem biometria → bloqueia assinatura até regularizar
```

**Alerta:** Se a pessoa já tem cautela aberta, exibe aviso mas não bloqueia a criação.

**Pendência cadastral:** não bloqueia avançar para materiais, mas alerta e oferece regularização inline (v1.4). Assinatura no passo 4 exige PIN ou biometria cadastrados.
...
| getPersonById(id) | UUID | { person, error } |
| createPerson(data) | PersonCreateInput | { success, id } |
| regularizePerson(id, data) | UUID, RegularizeInput | { success } \| { error } |
| updatePerson(id, data) | UUID, Partial | { success } |
| uploadRgPhoto(formData) | FormData | { url } |
| attachPhotos(personId, front?, back?) | UUID, URLs | { success } |
| checkPhotosPending(personId) | UUID | { pending, missing } |
| searchPersons(query) | string | Person[] (em `cautelas.ts`) |
...
components/
  ...
  MaterialForm.tsx                # Formulário de criação/edição de material (modal rolável)
  MaterialCautelaDetailModal.tsx  # Detalhe da cautela ativa ao clicar "Em Uso"
  PersonRegistrationWizard.tsx    # Wizard cadastro + modo regularização
...
lib/
  ...
  cautela-return-status.ts        # Lógica itemNeedsReturn (devolução parcial)
  material-active-detail.ts       # Mapa material → cautela ativa
  materials-page-data.ts          # Enriquecimento da listagem de materiais
  person-registration-status.ts   # Pendências de cadastro (fotos + biometria)
...
- `persons.phone` — campo WhatsApp/telefone (text; **v1.3:** obrigatório no cadastro completo)
...
### 17.13 Cadastro de pessoa v1.3 (planejado em código/schema)
- `persons.cpf` — text, único, NOT NULL (migration pendente)
- Validação no wizard e em `createPerson`: todos os campos §5.1 obrigatórios antes de salvar
- Bloqueio de cautela para pessoa com cadastro incompleto (sem fotos, CPF, WhatsApp ou e-mail)
...
## 19. Entregas v1.4 — 2026-06-06 (sessão de desenvolvimento)

### 19.1 Ambiente de teste e CI

- Alias estável **https://reserva-teste.vercel.app** apontando para deploy Preview com banco `teste_db`
- Workflow `.github/workflows/deploy-test.yml` para pipeline de teste
- Secrets GitHub configurados via `scripts/set-github-secrets.ps1` (Supabase teste, Vercel token)
- Login QA documentado: `qa.supervisor@reserva.test` (ver `docs/QA-E2E-TESTE-REPORT.md`)
- `GET /api/version` expõe ambiente para evitar confusão prod vs teste

### 19.2 UX Materiais e deep link de cautela

| Item | Arquivo(s) | Comportamento |
|------|------------|---------------|
| Modal rolável | `MaterialForm.tsx` | Corpo scrollável, rodapé fixo, Escape/backdrop |
| Lista responsiva | `MaterialsClient.tsx` | Cards mobile; ações touch-friendly |
| Detalhe "Em Uso" | `MaterialCautelaDetailModal.tsx` | Pessoa, operador, data, link cautela |
| Dados ativos | `material-active-detail.ts`, `materials-page-data.ts` | Query cautelas `open`/`partial` |
| Deep link | `CautelasClient.tsx`, `/cautelas?page` | Suporte a `?id=<cautelaId>` na URL |

**Commits de referência (teste):** `90b58c9` (feat materials UX), `0af2e49` (fix detalhe Em Uso — coluna `returned` inexistente).

### 19.3 Importação legado — cautelados 1º BPM (somente teste_db)

**Fonte:** DOCX `cautelados-1bpm-atualizada-2026-06-06.docx`  
**Scripts:** `scripts/import/parse-cautelados-docx.mjs`, `import-cautelados-test.mjs`

```
node scripts/import/parse-cautelados-docx.mjs --input <docx>
node scripts/import/import-cautelados-test.mjs          # dry-run
node scripts/import/import-cautelados-test.mjs --apply  # só teste_db
```

| Métrica | Resultado (06/06/2026) |
|---------|------------------------|
| Linhas CAUTELADO no DOCX | 141 |
| Pessoas novas | 118 |
| Pessoas reutilizadas | 2 (JHONNY, TESTE QA E2E) |
| Cautelas permanentes abertas/parciais | ~119 |
| Itens vinculados | ~134 (após limpeza de 6 duplicatas) |
| Seriais ausentes no estoque | 4 (ATAIDES, VALDSON, GOUVEIA, MACEDO) |

**Credenciais temporárias (pessoas importadas):**
- PIN: `0000` (trocar no balcão)
- E-mail placeholder: `pendente+<matrícula>@cadastro.reserva.local`

**Regras anti-duplicação no import:**
1. Pessoa por `registration_number` — não recria
2. Uma cautela `open`/`partial` por pessoa
3. Ignora item já vinculado (`person` + `material`)
4. Bloqueia material cautelado a **outra** pessoa (`materialHolder`)
5. Ignora linha repetida no mesmo grupo

Relatórios: `scripts/import/import-final-report.md`, `scripts/import/dry-run-report.md`.

> **Produção não foi alterada** nesta importação.

### 19.4 Regularização de cadastros pendentes

Implementado conforme §5.5:
- Badge **Pendente** clicável em `/persons`
- Link **Regularizar cadastro** no wizard de Nova Cautela (modal inline)
- Actions `getPersonById` + `regularizePerson`
- Helper `person-registration-status.ts`

### 19.5 Status em ambiente de teste (pós-sessão)

| Funcionalidade | Rota / componente | Status teste |
|----------------|-------------------|--------------|
| Listagem materiais responsiva | `/materials` | ✅ |
| Detalhe cautela via badge Em Uso | `MaterialCautelaDetailModal` | ✅ |
| Import cautelados 1º BPM | `scripts/import/*` | ✅ (teste_db) |
| Regularizar cadastro (Pessoas) | `/persons` → Pendente | ✅ |
| Regularizar cadastro (Cautela) | `CautelaWizard` passo 1 | ✅ |
| Deep link cautela | `/cautelas?id=` | ✅ |

### 19.6 Pendências remanescentes

- Promover v1.4 para **produção** após QA formal e aprovação explícita
- Cadastrar 4 seriais faltantes no estoque e reimportar (idempotente)
- Completar cadastros importados (fotos RG, biometria, e-mail real, troca de PIN)
- Campos §5.1 planejados (CPF obrigatório, bloqueio duro) — migration `persons.cpf` ainda pendente em código
- WhatsApp e Resend — continuam aguardando env vars (ver cabeçalho)

---

*Documento atualizado em 2026-06-06 — v1.4: ambiente QA, UX materiais, importação legado 1º BPM, regularização de cadastro pendente.*
