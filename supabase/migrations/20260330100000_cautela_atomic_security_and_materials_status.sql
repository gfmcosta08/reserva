-- 1) Status usados pelo app (returnItem, API) mas ausentes no CHECK anterior
...
-- 2) RPC: operador = auth.uid() (não confia em parâmetro do cliente)
DROP FUNCTION IF EXISTS public.create_cautela_atomic(uuid, uuid, text, text, uuid[]);
...
GRANT EXECUTE ON FUNCTION public.create_cautela_atomic(uuid, text, text, uuid[]) TO authenticated;

COMMENT ON FUNCTION public.create_cautela_atomic IS 'Cria cautela atomicamente; operator_id = auth.uid().';
