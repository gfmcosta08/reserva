          ...
          <div className="space-y-5">
            {hasFaceForSign && useFace ? (
              <>
                ...
                {/* Loading enquanto cria cautela */}
                {loading && (
                  <div className="flex items-center justify-center gap-2 py-3">
                    <Loader2 className="h-5 w-5 text-blue-500 animate-spin" />
                    <span className="text-sm text-blue-400 font-medium">Criando cautela...</span>
                  </div>
                )}

                {/* Erro após verificação facial falhar no servidor */}
                {pinError && !loading && (
                  <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                    <AlertTriangle className="h-4 w-4 text-red-500 mt-0.5 flex-shrink-0" />
                    <div className="flex-1">
                      <p className="text-xs font-medium text-red-400">{pinError}</p>
                      <button
                        type="button"
                        onClick={() => { setPinError(""); setStep(3) }}
                        className="text-xs text-blue-400 hover:text-blue-300 mt-1 underline"
                      >
                        Voltar ao resumo e tentar novamente
                      </button>
                    </div>
                  </div>
                )}
