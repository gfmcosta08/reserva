...
import { getPersons, getPersonById, deletePerson } from "@/app/actions/persons"
import PersonRegistrationWizard, {
  type PersonWizardEditTarget,
} from "@/components/PersonRegistrationWizard"
import { personHasRegistrationPending } from "@/lib/person-registration-status"
...
function toWizardTarget(person: Record<string, unknown>): PersonWizardEditTarget {
  return {
    id: String(person.id),
    ...
    function: (person.function as string | null) ?? null,
    phone: (person.phone as string | null) ?? null,
    rg_front_url: (person.rg_front_url as string | null) ?? null,
    rg_back_url: (person.rg_back_url as string | null) ?? null,
    face_descriptor: (person.face_descriptor as number[] | null) ?? null,
  }
}

function PersonsClientInner({ userRole = "operator" }: { userRole?: string }) {
  ...
  const openRegularize = useCallback(async (personOrId: string | Record<string, unknown>) => {
    setLoadingEdit(true)
    try {
      if (typeof personOrId === "string") {
        const { person, error } = await getPersonById(personOrId)
        if (!person) {
          alert(error || "Pessoa não encontrada")
          return
        }
        setRegularizeTarget(toWizardTarget(person))
      } else {
        setRegularizeTarget(toWizardTarget(personOrId))
      }
  ...
  useEffect(() => {
    ...
    if (!editId) return
    openRegularize(editId).then(() => {
      router.replace("/persons", { scroll: false })
    })
  }, [searchParams, openRegularize, router])
  ...
  return (
    <div className="p-8 space-y-8">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                ...
                persons.map((person) => {
                  const pending = personHasRegistrationPending(person)
                  return (
                    <tr key={person.id} className="group hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {pending ? (
                          <button
                            type="button"
                            onClick={() => openRegularize(person)}
                            disabled={loadingEdit}
                            title="Clique para regularizar o cadastro"
                            className="inline-flex items-center gap-1.5 rounded-full bg-yellow-500/10 px-3 py-1 text-xs font-bold text-yellow-500 border border-yellow-500/20 hover:bg-yellow-500/20 hover:border-yellow-500/40 transition-colors cursor-pointer"
                          >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            title="Editar / regularizar cadastro"
                          >
      ...
      {wizardOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <PersonRegistrationWizard
            person={regularizeTarget ?? undefined}
            onSuccess={() => {
              closeWizard()
              fetchPersons(search)
            }}
            ...
            initialData={
              !regularizeTarget && search.match(/^\d+$/) ? { rg: search } : { name: search }
            }
          />
        </div>
      )}
      ...
      {loadingEdit && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 pointer-events-none">
...
export default function PersonsClient({ userRole = "operator" }: { userRole?: string }) {
  return (
    ...
    >
      <PersonsClientInner userRole={userRole} />
    </Suspense>
  )
}
